@if(!empty($Topic))
    <article>
        <div class="card post-card pa-0 mb-4" style="height: 80%">
                    ...
                    <div class="video-container position-relative">
                        @if ($Topic->photo_file != "")
                            <img class="card-img-top"
                                 src="{{ URL::to('uploads/topics/'.$Topic->photo_file) }}"  width="40%" height="40%"
                        ...
                        @else
                            ...
                            @if($Topic->video_type ==1)
                                ...
                                ?>
                                <img class="card-img-top" src="{{ $img_url }}"  width="40%" height="40%"
                                     alt="{{ $title }}" loading="lazy"/>
                            @else
                                <div
...
{{--                audio--}}
                <div class="audio-container position-relative">
                    @if($Topic->photo_file !="")
                        <img class="card-img-top" loading="lazy"  width="40%" height="40%"
...
{{--                photo slider--}}
                <a href="{{ $topic_link_url }}">
                    <div class="image-container position-relative" style="height: 200px;width: 200px" >
                        @if($Topic->photo_file !="")
                            <img class="card-img-top" loading="lazy"  width="40%" height="40%"
                                 src="{{ URL::to('uploads/topics/'.$Topic->photo_file) }}"
                                 alt="{{ $title }}"/>
                        ...
                        @else
                            @foreach($Topic->photos->random(1) as $photo)
                                <img class="card-img-top" loading="lazy"
                                     src="{{ URL::to('uploads/topics/'.$photo->file) }}"  width="40%" height="40%"
                                     alt="{{ $title }}"/>
                            @endforeach
                        @endif
...
{{--                one photo--}}
                <a href="{{ $topic_link_url }}">
                    <div >
                        @if($Topic->photo_file !="")
                            <img class="" loading="lazy"  width="40%" height="40%"
                                 src="{{ URL::to('uploads/topics/'.$Topic->photo_file) }}"
                                 alt="{{ $title }}"/>
                        @endif
                    </div>
            ...
            @endif
            <div class="mt-3" style="padding: 5px ;width: 100%">
                <a href="{{ $topic_link_url }}">
                    <h4 class="card-title" >
                        @if($Topic->icon !="")
...
{{--                Additional Feilds--}}
                ...
                @if(strip_tags($Topic->$details) !="")
                    <p class="card-text mb-0 mt-2" style="padding: 5px;height: 200px">
                        {!! mb_substr(strip_tags($Topic->$details),0, 180)."..." !!} <a
                            href="{{ $topic_link_url }}" class="read-more-link" style="color: #0a58ca">{{ __("frontend.moreDetails") }}</a>
                    </p>
                @endif
            </div>
        </div>

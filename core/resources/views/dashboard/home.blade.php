...
@push("after-styles")
    ...
    <script src="{{ asset('js/token-refresh.js') }}"></script>
...
{{--    @dd($serchcount)--}}
    <div class="padding p-b-0">
        <div class="margin">
            <h5 class="m-b-0 _300">
                {{ __('backend.hi') }} <span class="text-primary">{{ Auth::user()->name }}</span>, {{ __('backend.welcomeBack') }}
            </h5>
            @if(@Auth::user()->permissionsGroup->webmaster_status)
                <div class="button-container">
                    <a  href="{{ route('backup.viewcheck') }}" class="btn btn-primary">{{__('backend.backup')}}</a>
        ...
        <style>
            ...
            .button-container {
                text-align: right; /* يجعل العناصر داخل هذا الديف محاذاة لليمين */
            }
                            ...
                            @if (config('smartend.geoip_status'))
                                <div class="col-xs-12">
                                    <div class="row-col box-color text-center primary">
                                        ...
                                        <div class="row-cell p-a ">
                                            {{ __('backend.searches') }}
                                            <h4 class="m-a-0 text-md _600"><a href>{{$serchcount->countsearch}}</a></h4>
                                        </div>
                                    </div>
                                </div>

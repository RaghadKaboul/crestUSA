...
class WebmasterSettingsController extends Controller
{
    ...
    public function edit()
    {
        ...
        $Newses = Topic::where('webmaster_id', 3)->orderby('row_no', 'asc')->get();
        $Links = Topic::where('webmaster_id', 28)->orderby('row_no', 'asc')->get();
        $Videos = Topic::where('webmaster_id',5 )->orderby('row_no', 'asc')->get();

        $Countries = Country::orderby('title_' . @Helper::currentLanguage()->code, 'asc')->get();
        $Languages = Language::orderby('id', 'asc')->get();
        $Pages = Topic::where('webmaster_id', 1)
            ->orderBy('row_no', 'asc')
            ->get();
        $excludedIds = [1];
        $Topics=WebmasterSection::whereNotIn('id',$excludedIds)
            ->orderBy('row_no', 'asc')
            ->get();
        $WebmasterSetting = WebmasterSetting::find(1);
        if (!empty($WebmasterSetting)) {
            return view("dashboard.system_settings.home", compact("WebmasterSetting", "GeneralWebmasterSections", "ParentMenus", "WebmasterBanners", "ContactsGroups", "SitePages", "PermissionsGroups", "Countries", "Languages","Newses","Links","Videos","Pages","Topics"));
    ...
    {
        $topics = Topic::where('webmaster_id',1)->where('status',1)->get();
        // Loop through the pages array in the request
        foreach ($request->get('pages') as $pageid => $content) {
          foreach ($topics as $topic)

            if ($topic->id==$pageid) {
                // Update the fields
                $topic->pages_content1 = $content['content1'];
                $topic->pages_content2 = $content['content2'];
                $topic->pages_content3 = $content['content3'];
                $topic->pages_content_link = $content['content_link'];
                $topic->pages_content_video = $content['content_video'];

                $topic->save(); // Don't forget to save the changes
            }
        }
        ...
        if (!empty($WebmasterSetting)) {
            ...
            $WebmasterSetting->home_video_section_id = $request->home_video_section_id;
            $WebmasterSetting->home_link_section_id = $request->home_link_section_id;
